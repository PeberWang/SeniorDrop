"""Convert setup.bat / start.bat / update.bat to GBK so Windows cmd parses them correctly.
...
本脚本把含中文的 .bat 从 UTF-8 转为 GBK 并删除多余的 chcp 65001 行。
已经是 GBK 的文件会自动跳过（幂等，可重复跑）。
"""
import sys
from pathlib import Path

TARGETS = ["setup.bat", "start.bat", "update.bat"]
...
def convert(path: Path) -> None:
    try:
        ...
        print(f"skip (not UTF-8, likely already GBK): {path.name}")
        return
    lines = [
        line
